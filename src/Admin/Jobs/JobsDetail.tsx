...
import ApplicationModal from "../../UserView/Recruitment/ApplicationModal";
...
const JobDetail: React.FC = () => {
  ...
  const fetchJobDetail = async (jobId: string, forceRefresh = false) => {
    const now = Date.now();
    
    // Chỉ fetch lại nếu đã quá 30 giây hoặc force refresh
    if (!forceRefresh && job && (now - lastFetchTime) < 30000) {
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <SidebarInfoCard title="Thống kê ứng tuyển">
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                  <span className="text-gray-600">Ứng viên mới (7 ngày):</span>
              ...
              <button
                ...
                className="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white py-3 px-4 rounded-xl transition-all duration-300 ease-in-out font-medium text-base shadow-md hover:shadow-lg"
              >
                Xem danh sách ứng viên
              </button>
              <ApplicantsList
                isOpen={showApplicantsList}
                onClose={() => setShowApplicantsList(false)}
                jobId={job?.id || ""}
                jobTitle={job?.title || ""}
